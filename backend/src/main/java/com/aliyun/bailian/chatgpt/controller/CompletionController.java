...
public class CompletionController {
    ...
    public Flux<Result<CompletionResponseDTO>> complete(@RequestBody CompletionRequestDTO completionRequest,
        ...
        try {
            ...
            if (StringUtils.isBlank(requestId) || StringUtils.isBlank(content) || StringUtils.isBlank(code) || StringUtils.isBlank(sessionType)) {
    ...
    public Result<UploadPolicyResponseDTO> getUploadPolicy(@RequestBody UploadPolicyRequestDTO request) {
        ...
        try {
            ...
            LogUtils.monitor(request.getRequestId(), "CompletionController", "getUploadPolicy", null, start, request, result);
            return result;
        } catch (BizException e) {
            LogUtils.monitor(request.getRequestId(), "CompletionController", "getUploadPolicy", "exception", start, request, e);
    ...
    public Result<ImportDocumentResponseDTO> importDocument(@RequestBody ImportDocumentRequestDTO request) {
        ...
        try {
            ...
            LogUtils.monitor(request.getRequestId(), "CompletionController", "importDocument", null, start, request, result);
            return result;
        } catch (BizException e) {
            LogUtils.monitor(request.getRequestId(), "CompletionController", "importDocument", "exception", start, request, e);
            return Result.error(request.getRequestId(), e.getErrorCode(), e.getMessage());
        } catch (Exception e) {
            LogUtils.monitor(request.getRequestId(), "CompletionController", "importDocument", "exception", start, request, e);
